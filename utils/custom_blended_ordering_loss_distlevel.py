...
class Opt1_DistBlendedOrderingLoss(nn.Module):
    ...
    def forward(self, x, annotator_matrix, num_dist_types, num_levels, anchor_negatives= "min"):  # x => B x 17 x 4096
        ...
        positive_indices = nanargmax2(final_masked_annotation_matrix, dim=2, second_highest=False) # Find the indices of the max values in each row (B x 17)
        if anchor_negatives == "min":
            ...
            negative_indices = nanargmax2(final_masked_annotation_matrix, dim=2, second_highest=True) # Find the indices of the second highest values in each row (B x 17)
...
def nanargmax2(tensor, dim=None, second_highest=True):
    ...
    _, indices = torch.topk(tensor_nan_to_num, 2, dim=dim, largest=True, sorted=True)
